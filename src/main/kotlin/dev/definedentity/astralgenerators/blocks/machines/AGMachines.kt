...
object AGMachines {
    ...
    val STEAM_TURBINE_CONTROLLER =
        REGISTRATE.block("steam_turbine_controller", ::Disassembler).lang("Steam Turbine Controller")
    ...
    val BOILER_CONTROLLER =
        REGISTRATE.block("boiler_controller", ::Disassembler).lang("Boiler Controller")
            .blockEntity { type, pos, state ->
                DisassemblerEntity(type, pos, state)
            }.build().blockstate { ctx, prov ->
                for ((direction, rotationY) in AGDirections.directions) {
